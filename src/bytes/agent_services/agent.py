...
def get_context(query: str) -> tuple[str, list[dict]]:
    results = retriver.vectorstore.similarity_search(query, k=10)
    source_json = []
    prompt = ""
    for doc in results:
...
def extract_page_excerpt(page: int) -> str:
    results = retriver.vectorstore.similarity_search_by_metadata(metadata={"page_number": page})
    if not results:
